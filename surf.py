...
def get_active_beach():
    if len(sys.argv) > 1 and sys.argv[1].strip():
        ...
        print(f"חוף חדש נבחר ונשמר: {beach}")
        return beach
...
def extract_summary_and_astro(anchor):
    ...
    for _ in range(40):
        ...
        # חילוץ זריחה/שקיעה ללא מילים או אימוג'ים מיותרים
        if "זריחה" in t and "שקיעה" in t:
            ...
            # חילוץ טקסט הסיכום (עד המילה "זריחה") וניקוי אימוג'ים
            text_m = re.search(r"(בבוקר.*?)(?=זריחה:|$)", t)
            if text_m:
                clean_text = text_m.group(1).replace("☀️", "").replace("🌤", "").replace("⛅", "").replace("🌤️", "").strip()
...
def get_live_data(beach_slug):
    ...
    print(f"1. מוריד נתונים מ-GoSurf עבור {beach_slug}...", flush=True)
    ...
    # חילוץ גאות ושפל (בדרך כלל מופיע באזור העליון)
    tide_str = ""
    high_m = re.search(r"גאות:\s*(\d+:\d+)", page_text)
    low_m = re.search(r"שפל:\s*(\d+:\d+)", page_text)
    if high_m and low_m:
        tide_str = f" · גאות {high_m.group(1)} · שפל {low_m.group(1)}"
    elif high_m:
        tide_str = f" · גאות {high_m.group(1)}"
    elif low_m:
        tide_str = f" · שפל {low_m.group(1)}"
    ...
    for d in range(7):
        ...
        while curr:
            ...
            if hasattr(curr, "select"):
                for tr in curr.select(".chart_tr, tr"):
                    ...
                    day_rows.append({
                        "hour_num": int(hour_str), "hour": hour_str, "wave": wave_txt,
                        "desc": wave_desc, "wind": wind_str, "swell": swell_str, "icon": w_icon,
                    })
        ...
        forecast.append({
            "day": day_name, "date": day_date, "wave": wave_12 if wave_12 > 0 else 40,
            ...
            "stars": stars_formatted, "icon": icon_12,
        })

    beach_title = BEACHES.get(beach_slug, beach_slug)
    return {"beach_name": beach_title, "current": current_data, "forecast": forecast}

if __name__ == "__main__":
    active_beach = get_active_beach()
    live_data = get_live_data(active_beach)
    print(f"שולח נתונים ל-TRMNL עבור {active_beach}...", flush=True)
    res = requests.post(TRMNL_WEBHOOK_URL, json={"merge_variables": live_data}, timeout=10)
    print(f"תגובת השרת: {res.status_code}", flush=True)
